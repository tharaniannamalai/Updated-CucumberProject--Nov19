...
public class stepdef_data_hooks extends Utility_Files{
	...
	public static Logger log = Logger.getLogger(Adactin_Runner_With_Data_Hks.class);
	...
	@When("user Click On The Search Btn, It Navigates To {string}")
	...
	@When("user Click On The Continue Btn, It Navigates To {string}")
	...
	@When("user Click On The Book Now Btn, It Navigates To {string}")
	...
	@When("user Click On Booked Itinerary Link, It Navigates To {string}")
	public void user_click_on_booked_itinerary_link_it_navigates_to(String expected_Title) throws IOException, InterruptedException {
	...
	@When("user Select The {string} In Hotels Field2")
	public void user_select_the_in_hotels_field2(String hotelName) {
	...
	@When("user Select The {string} In Number of Rooms Field")//Check here
	public void user_select_The_in_number_of_rooms_field(String noOfRoom) {
	    
		select_By_Visibletext(manage.getSearchHotel().getNoOfRooms(), noOfRoom);
	}
	
	@When("user Enter The Checkindate {string} In Check In Date Field")
	public void user_enter_the_checkindate_in_check_in_date_field(String checkInDate) {
		
		send_Keys(manage.getSearchHotel().getCheckInDate(), checkInDate);
	}
	@When("user Enter The Checkoutdate {string} In Check Out Date Field")
	public void user_enter_the_checkoutdate_in_check_out_date_field(String checkOutDate) {
		
		send_Keys(manage.getSearchHotel().getCheckOutDate(), checkOutDate);
	}
	@When("user Select The Adultsperroom {string} In Adults per Room Field")
	public void user_select_the_adultsperroom_in_adults_per_room_field(String noOfAdult) {
		
		int noOfAdult1 = Integer.parseInt(noOfAdult); //Conveting to int bec of selectByIndex
		select_By_Index(manage.getSearchHotel().getNoOfAdult(), noOfAdult1);
	}
	
	@When("user Select The Childrenperroom {string} In Children per Room Field")
	public void user_select_the_childrenperroom_in_children_per_room_field(String noOfChildren) {
		
		select_By_Value(manage.getSearchHotel().getNoOfChildren(), noOfChildren);
	}
	@When("user Click On Search Btn, It Navigates To {string}")
	public void user_click_on_search_btn_it_navigates_to(String expected_Title) {
	...
	@When("user Click On Radio Btn, In Select Field")
	public void user_click_on_radio_btn_in_select_field() {
		
		click_On_Element(manage.getSlctHotel().getRadioBtn());
	}
	
	@When("user Click On Continue Btn, It Navigates To {string}")
	public void user_click_on_continue_btn_it_navigates_to(String expected_Title) {
		click_On_Element(manage.getSlctHotel().getContinuBtn());
		String actual_Title = driver.getTitle();
		Assert.assertEquals(actual_Title , expected_Title);
	}
	@When("user Enter Firstname {string} In First Name Field")
	public void user_enter_firstname_in_first_name_field(String firstName) {
		
		send_Keys(manage.getBookHotel().getFirstName(), firstName);
	}
	@When("user Enter Lastname {string} In Last Name Field")
	public void user_enter_lastname_in_last_name_field(String lastName) {
		
		send_Keys(manage.getBookHotel().getLastName(), lastName);
	}
	@When("user Enter Billingaddress {string} In Billing Address Field")
	public void user_enter_billingaddress_in_billing_address_field(String billingAddress) {
		
		send_Keys(manage.getBookHotel().getBillingAddress(), billingAddress);
	}
	@When("user Enter Creditcardno {string} In Credit Card No Field")
	public void user_enter_creditcardno_in_credit_card_no_field(String creditCardNo) {
		
		send_Keys(manage.getBookHotel().getCreditCardNo(), creditCardNo);
	}
	
	@When("user Select Creditcardtype {string} In Credit Card Type Field")
	public void user_select_creditcardtype_in_credit_card_type_field(String creditCardType) {
		
		select_By_Visibletext(manage.getBookHotel().getCreditCardType(), creditCardType);
	}
	
	@When("user Select Month {string} In Expiry Date Field")
	public void user_select_month_in_expiry_date_field(String expiryMonth) {
		
		select_By_Visibletext(manage.getBookHotel().getExpiryMonth(), expiryMonth);
	}
	@When("user Select Year {string} In Expiry Date Field")
	public void user_select_year_in_expiry_date_field(String expiryYear) {
		
		select_By_Visibletext(manage.getBookHotel().getExpiryYear(), expiryYear);
	}
	@When("user Enter Cvvnumber {string} In CVV Number Field")
	public void user_enter_cvvnumber_in_cvv_number_field(String ccvNo) {
